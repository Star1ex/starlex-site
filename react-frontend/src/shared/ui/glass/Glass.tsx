...
function GlassImpl(
  { as, variant, depth, interactive, className, children, ...rest }: GlassOwnProps & Record<string, unknown>,
  ref: Ref<Element>,
) {
  ...
  return (
    <Comp
      ref={ref}
      className={cn(glass(variantProps), className)}
      {...rest}
    >
      {children}
    </Comp>
 ...
 * lives in `glass.css`; this component only composes the variant classes.
 * Polymorphic + ref-forwarding so it can wrap framer-motion elements.
 */
