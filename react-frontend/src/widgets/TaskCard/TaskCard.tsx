...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  in_progress: { label: 'In Progress', color: 'bg-gray-200 text-gray-800' },
...
const assignedUsers = task.assignedTo
  ...
  return (
    <article className="group bg-white rounded-2xl shadow-sm border border-gray-200 hover:shadow-lg hover:-translate-y-1 hover:scale-[1.02] transition-all duration-300 p-6 h-full">
      <div className="flex justify-between items-start mb-4 gap-3">
        ...
        <div className="flex gap-2 flex-shrink-0">
          <select
            ...
            aria-label={`Task status: ${statusConfig[status]?.label || status}`}
          >
      ...
      {assignedUsers.length > 0 && (
        <div className="flex items-center gap-2" aria-label={`${assignedUsers.length} users assigned`}>
